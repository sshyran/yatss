...
function events($page_number=0)
{
	...
	if ($limitfrom>=$count_of_rows) {
		$limitfrom=0; $page_number=-1;
	}
	$myarray=executeQuery('select event_id, name, date_format(date, "%Y-%m-%d %H:%i") as "event date", description from view_event_info where date > CURRENT_TIMESTAMP group by event_id order by date asc limit '.$limitfrom .', '. $step);
	...
	if (round($count_of_rows / $step)>$page_number) {
		$t->assign('next_page',$page_number+1);		
	}
	if ($page_number) {
		$t->assign('prev_page',$page_number-1);
	}
}
